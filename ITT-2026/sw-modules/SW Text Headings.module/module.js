// ===============================
// Smithworks' SW Text Headings module.js
...
(function() {
  ...
  function runEmbedLogic() {
    // Find oEmbed containers (Embed URL) and embed containers (Embed HTML) inside our video background wrapper
    var oembedContainers = document.querySelectorAll('.sw-text-headings--has-bg-video .oembed_container');
    var embedContainers = document.querySelectorAll('.sw-text-headings--has-bg-video .embed_container');
    ...
    function loadOEmbed(container) {
      ...
      // Support both dataset and getAttribute (dataset.embedUrl is camelCase for data-embed-url)
      ...
      request.onload = function() {
        if (request.status >= 200 && request.status < 400) {
          try {
            ...
            // Background video: autoplay + mute for browser policy (YouTube/Vimeo)
            ...
            console.error('SW Text Headings oEmbed: parse error', e);
          }
        } else {
          console.error('SW Text Headings oEmbed: server returned ' + request.status);
        }
      };
      request.onerror = function() {
        console.error('SW Text Headings oEmbed: network error');
      };
      request.send();
    }
  ...
  // Run when DOM is ready so the module markup is present
